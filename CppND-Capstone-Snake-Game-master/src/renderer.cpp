...
// Takes the array of blocks for a specific letter, along with the upper
// left grid location, and draws the letter accordingly
...
void Renderer::RenderStart(int messageStartPos) {
	...
	for (int i = 0; i < playerOne.size(); i++) {
		Renderer::drawLetter(playerOne[i], 10 + playerOneHorz[i], 8 + playerOneVert[i]);
	}

	// Render P2 Controls
	for (int i = 0; i < playerTwo.size(); i++) {
		Renderer::drawLetter(playerTwo[i], 20 + playerTwoHorz[i], 8 + playerTwoVert[i]);
	}
